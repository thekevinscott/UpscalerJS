/**
 * Patch/tile math — **verbatim** copy of the tfjs implementation
 * (`packages/upscalerjs/src/shared/image-utils.ts`). Backend-agnostic.
 *
 * This is the kind of logic that can be shared across TF.js and ONNX backends,
 * and in a real migration we'd hoist this to `packages/shared` rather than
 * duplicate it.
 */
import type { Coordinate, Patch, } from './types';
...
const get1DPatch = (total: number, idx: number, patchSize: number, padding: number) => {
  ...
  return {
    pre: { origin: preOrigin, size: patchSize, },
    post: { origin: postOrigin, size: postSize, },
    increment,
  };
};
...
export const getPatchesFromImage = ([width, height,]: Coordinate, patchSize: number, padding: number): Patch[][] => {
  ...
  while (y < height) {
    ...
    while (x < width) {
      ...
      row.push({
        pre: { origin: [preOriginY, preOriginX,], size: [preSizeY, preSizeX,], },
        post: { origin: [postOriginY, postOriginX,], size: [postSizeY, postSizeX,], },
      });
      x += xIncrement;
    }
    patches.push(row);
