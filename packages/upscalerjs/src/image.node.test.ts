...
describe('Image', () => {
  ...
  describe('getImageAsPixels', () => {
    it('handles a uint array', async () => {
      ...
      const result = await getImageAsPixels(image);
    ...
    it('handles a local string path to a file', async () => {
      const FLOWER = path.resolve(FIXTURES, 'flower-small.png');
      const result = await getImageAsPixels(FLOWER);
      expect(result.canDispose).toEqual(true);
      expect(result.tensor.shape).toEqual([1,16,16,4,]);
    });
    ...
    it('handles a string path to an http-hosted file', async () => {
      ...
      const result = await getImageAsPixels(imagePath);
      expect(result.canDispose).toEqual(true);
      expect(result.tensor.shape).toEqual([1,16,16,4,]);
    });
    ...
    it('handles an invalid input', async () => {
      await expect(() => getImageAsPixels(123 as any))
        .rejects
