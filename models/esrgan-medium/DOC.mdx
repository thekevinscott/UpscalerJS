...
## Goals for the model
ESRGAN Medium is meant to strike a balance between latency and performance, and aims to be the midpoint between `@upscalerjs/esrgan-slim` and `@upscalerjs/esrgan-thick`.

Future experimentation may result in additional hyperparameter explorations, along with pruning of the neural network. This may result in a slight decrease in performance, but only if accompanied by a substantial reduction in latency.
