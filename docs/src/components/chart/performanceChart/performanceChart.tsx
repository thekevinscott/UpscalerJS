...
export default function PerformanceChart({ databasePath, package: packageName }: { package?: string; databasePath: string }) {
  const [metrics, setMetrics] = useState<Metric[]>(getDefaultMetrics());
  const [datasets, setDatasets] = useState<Dataset[]>(getDefaultDatasets());
  const [activeDataset, setActiveDataset] = useState<ActiveDataset>(getDefaultActiveDataset(datasets));
