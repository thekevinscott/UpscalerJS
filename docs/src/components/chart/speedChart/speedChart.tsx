...
export default function SpeedChart({ databasePath, package: packageName }: { package?: string, databasePath: string }) {
  const [devices, setDevices] = useState<Device[]>(getDefaultDevices());
  const [activeDevice, setActiveDevice] = useState<{ device: Device, asc: boolean }>(getDefaultActiveDevice());
