...
import { TFJS_LIBRARY_TARGET_ERROR, getTFJSLibraryTarget } from './get-tfjs-library-target';
...
describe('getTFJSLibraryTarget', () => {
  ...
  it('loads the correct package json from the right directory', async () => {
    ...
    expect(await getTFJSLibraryTarget('foo')).toBe('browser');
  ...
  it('returns node for @tensorflow/tfjs-node', async () => {
    ...
    expect(await getTFJSLibraryTarget('foo')).toBe('node');
  });
  ...
  it('returns node-gpu for @tensorflow/tfjs-node-gpu', async () => {
    ...
    expect(await getTFJSLibraryTarget('foo')).toBe('node-gpu');
  });

  it('throws if no dependencies are found', async () => {
    makeMock({
    });
    await expect(() => getTFJSLibraryTarget('foo')).rejects.toThrow(TFJS_LIBRARY_TARGET_ERROR('foo'));
  });
});
